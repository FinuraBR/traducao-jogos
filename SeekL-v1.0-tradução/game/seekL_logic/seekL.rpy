...
Use para selecionar colunas específicas (separadas por vírgula) ou todas as colunas de uma vez (*).
    {/size}{/font}""",
        """{font=HELLO.ttf.ttf}{size=25}2. FROM 
Indica de qual tabela você quer ver os dados. {color=ffffff69}

EX: SELECT * 
    FROM tabela.exemplo 

EX: SELECT nome_hacker, fruta_favorita 
    FROM tabela.exemplo 
    {/color}{/size}{/font}""", 
    """{font=HELLO.ttf.ttf}{size=25}3. WHERE 
Filtre seus dados para os registros que importam. As colunas usadas no filtro sempre serão exibidas. 
Pode usar até duas condições na cláusula WHERE, separadas por um OR ou AND. 
WHERE deve aparecer no final da consulta. {color=ffffff69}

EX: SELECT * 
    FROM tabela.exemplo 
    WHERE nome_hacker = 'incri'

EX: SELECT nome_hacker  
    FROM tabela.exemplo 
    WHERE numero_de_hacks > 15 
        and numero_de_hacks < 45 
    {/color}{/size}{/font}""", 
    """{font=HELLO.ttf.ttf}{size=25}4. JOIN 
Junte duas tabelas que compartilham uma coluna em comum. A coluna em comum sempre será exibida.
JOIN deve aparecer depois da declaração FROM.{color=ffffff69}

EX: SELECT * 
    FROM receita.contatos
    JOIN govaz.casamentos  

EX: SELECT * 
    FROM receita.contatos
    JOIN segredobeijo.usuarios 
    WHERE nome_completo = 'bruce johnson'
    {/color}{/size}{/font}""", 

    """{font=HELLO.ttf.ttf}{size=25}5. EXEC
Executa uma função com ou sem um parâmetro. Não funciona com nenhuma outra consulta seekL.{color=ffffff69}

EX: EXEC out('bruce.johnson@copmail.com')
    {/color}{/size}{/font}""",

    """{font=HELLO.ttf.ttf}{size=25}MAIS EXEMPLOS {color=ffffff69}

EX: SELECT telefone, nome_completo, num_distintivo 
    FROM receita.contatos
    JOIN govaz.info_policia
    WHERE num_distintivo = 55242  

EX: SELECT apelido_ss, telefone  
    FROM receita.contatos
    JOIN segredobeijo.usuarios 
    WHERE nome_completo = 'bruce johnson'
    {/color}{/size}{/font}"""
...
screen seekL_ui: 
    ...
    button: 
        ...
        if seekL_window_active == 1 and seekL_text_send != "": 
            background None
            text "{font=Teko-VariableFont_wght.ttf}{size=44}EXECUTAR" color "#000000"
        else: 
            background None
            text "{font=Teko-VariableFont_wght.ttf}{size=44}EXECUTAR" color "#d0ffd1"
    ...
    button: 
        ...
        if seekL_window_active == 1 and seekL_text_send != "": 
            background None
            text "{font=Teko-VariableFont_wght.ttf}{size=45}LIMPAR"  color "#95ec96"
        else: 
            background None
            text "{font=Teko-VariableFont_wght.ttf}{size=45}LIMPAR"  color "#3f3f3f"
        ysize seekL_button_height
        if seekL_window_active == 1:
...
screen game_over_text: 
    text "fim: seek{color=b3b3af80}Derrota{/color}": 
        xalign 0.5 
        yalign 1.0 
...
screen game_over_neutral_text: 
    text "fim: seek{color=ecde8f80}Vida{/color}": 
        xalign 0.95 
        yalign 1.0 
        font "Teko-VariableFont_wght.ttf"
        size 100
...
screen game_over_good_text: 
    text "fim: seek{color=f57cdf80}Amor{/color}": 
        xalign 0.95 
        yalign 1.0 
        font "Teko-VariableFont_wght.ttf"
        size 100
