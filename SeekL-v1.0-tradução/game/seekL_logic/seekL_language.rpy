...
screen horn_time: 
    timer 1.0 action Function(chat_message, "SYSTEM: " + horn_name + " DIZ PRA BUZINAR"), Play("honk", horn_sound,loop=False), Function(can_we_horn_more_sire), Hide("horn_time")
        ...
        if t != "":
            ...
            if loc_exec != "":
                ...
                if loc_exec_start != "" and loc_exec_end != "": 
                    ...
                    error_msg = "ERRO: NÃO FORAM ENCONTRADOS PARÊNTESES PARA A FUNÇÃO"
                ...
                if error_msg == "": 
                    if func_name == "out": 
                        ...
                        if func_dial == "5554484746": 
                            ...
                            func_name = func_name + "\n\nO NÚMERO INSERIDO NÃO PODE SER DISCADO."
                    elif func_name == "horn": 
                        chat_message("SYSTEM: THRIM DIZ PRA BUZINAR")
                    ...
                    if player_is_waiting: 
                        # "#8d8d8d"
                        seekL_output = ["{color=ee6464}"+error_msg+"{/color}\n\n\n\n\n{color=8d8d8d}(você já checou a guia do seekL?){/color}"]
                    else: 
                        seekL_output = ["{color=ee6464}"+error_msg+"{/color}"]
                    previous_commands.append("{color=945050}{size=20}SEM SUCESSO{/size}{/color}\n"+t_og)
                ...
                if " group by " in t or " order by " in t or " distinct " in t or " inner join " in t or " left join " in t or " right join " in t or "!=" in t: 
                    error_msg = "ERRO: uauuuuuu olha só esse usuário de SQL de verdade \nuauuuuuuuuuu vc é muuuuuito habilidosooooo \nME MOSTRA TODOS OS SEUS TRUQUES ESPECIALISTA EM SQL UAUUUUUUU \n\ndesculpa mas vc não pode usar o seguinte aqui:\nGROUP BY, ORDER BY, DISTINCT, INNER JOIN,\nLEFT JOIN, RIGHT JOIN, !=, +MAIS"

                if "==" in t and error_msg == "": 
                    error_msg = "ERRO: '==' INVÁLIDO. VOCÊ QUIS DIZER '='?"
                if " amd " in t and error_msg == "": 
                    error_msg = "ERRO: 'AMD' INVÁLIDO. VOCÊ QUIS DIZER 'AND'?"
                
                if " and " in t and error_msg == "": 
                    loc_bad_and = t.index(" and ")
                    if loc_from != "": 
                        if loc_bad_and < loc_from: 
                            error_msg = "ERRO: 'AND' SÓ PODE SER USADO EM WHERE. \nUSE VÍRGULAS EM SELECT PARA ESCOLHER COLUNAS."
                if " or " in t and error_msg == "": 
                    loc_bad_or = t.index(" or ")
                    if loc_from != "":
                        if loc_bad_or < loc_from: 
                            error_msg = "ERRO: 'OR' SÓ PODE SER USADO EM WHERE. \nUSE VÍRGULAS EM SELECT PARA ESCOLHER COLUNAS."
                ## error hunting 
                ...
                if sort_order: 
                    ...
                    for x in range(len(sort_order_test)): 
                        if x == 0 and sort_order_test[x][0] != "select": 
                            error_msg = "ERRO: ORDEM DE COMANDO INCORRETA"
                ...
                if loc_select != "" and loc_from != "": 
                    ...
                    cols_list = []
                    error_msg = "ERRO: SINTAXE INCORRETA"
                ...
                if table_name in tables and error_msg == "": 
                    ...
                    if join_name not in tables and join_name != "": 
                        error_msg = "ERRO: TABELA DE JOIN NÃO ENCONTRADA-\n" + join_name 
                    elif join_name != "": 
                    ...
                    if not cols_final and error_msg == "": 
                        error_msg = "ERRO: NOMES DE COLUNA NÃO ENCONTRADOS"
                elif error_msg == "": 
                    error_msg = "ERRO: NOME DA TABELA NÃO ENCONTRADO -\n" + table_name
                ...
                if loc_where != "" and error_msg == "":
                    ...
                    for wp in where_parts:
                        ...
                        if ">=" in wp: 
                            ...
                            split_me = "<"
                        if split_me == "": 
                            error_msg = "ERRO: FALTANDO DIRECIONAL NO WHERE \n(>, >=, =, <=, <)"
                        else: 
                        ...
                        if error_msg == "": 
                            ...
                            if where_column == "" or (where_column not in tables[table_name].keys() and join_name == ""): 
                                error_msg = "ERRO: FALTANDO COLUNA VÁLIDA NO WHERE"
                            elif join_name != "": 
                            ...
                            if where_value == "": 
                                error_msg = "ERRO: FALTANDO VALOR VÁLIDO NO WHERE"
                            else: 
                ...
                if not join_columns and join_name != "" and error_msg == "": 
                    error_msg = "ERRO: NENHUMA COLUNA EM COMUM ENTRE AS TABELAS"
                    ...
                    for wlv in range(len(where_column_list)):
                        ...
                        if where_split_list[wlv] == "=": 
                            if is_float(where_value_list[wlv]):
                                ...
                                error_msg = "ERRO: FALTANDO ASPAS NA STRING DE VALOR DO WHERE"
                            ...
                            try:
                                ...
                                error_msg = "ERRO: NÃO É POSSÍVEL APLICAR CONDIÇÃO WHERE \nA UMA COLUNA NÃO-NUMÉRICA"
                        else: 
                            error_msg = "ERRO: NÃO É POSSÍVEL APLICAR CONDIÇÃO WHERE \nA UM VALOR NÃO-NUMÉRICO"
